...
def test_1():
    ...
    tests = ['test_log.txt 10', 'abc.txt 10', 'abc.txt', 'test_log.txt aa']
    answer = [
        {'filename': 'test_log.txt', 'nftd': 10, 'error': None},
        {'filename': '', 'nftd': 0, 'error': 'File not found (no file abc.txt)'},
        {'filename': '', 'nftd': 0, 'error': 'Number of arguments error (except 2 get 1)'},
        {'filename': 'test_log.txt', 'nftd': 0, 'error': 'Argument 2 must be positive integer (get aa)'}
    ]
    ret = []
    for i in tests:
