...
fn execute(file_name: &str, gen_file_count: &str, output_format: &str, ans_exec_cmd: &str, id_base: &str) -> i32 {
    ...
    let id_base: u8 = if id_base.len() != 0 { match id_base.parse() {
        Ok(r) => { r }
        Err(_) => {
            helper_func::print_msg("the arguemnt of --id-base must be an positive integer or zero");
            return 2;
        }
    }} else { 0 };
    ...
    for count in 0..gen_file_count {
        ...
        let fn_template = if output_format.len() != 0 {
                output_format.replace(
                    &id_info.1, 
                    &match make_id(count + id_base, id_info.0) {
                        Ok(r) => { r }
                        Err(c) => {
        ...
        if ans_exec_cmd.len() != 0 {
            let ans_text = match subprocess::Exec::shell(ans_exec_cmd)
