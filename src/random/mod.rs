...
impl Gener {
    ...
    pub fn gen_i(&mut self, vi: &Vec<(i64, i64)>) -> Result<(), RuntimeError> {
        ...
        for (id, i) in vi.iter().enumerate() {
            ...
            let q: u64 = ((self.quantity as f64 * weight[id] as f64) / (self.i_weight as f64)).ceil() as u64;
    ...
    pub fn gen_f(&mut self, vf: &Vec<(f64, f64)>, pn: f64) -> Result<(), RuntimeError> {
        ...
        for i in vf.iter() { 
            let w: u128 = (i.1 - i.0).ceil() as u128;
            weight.push(w); 
            self.f_weight += w;
        }
        ...
        for (id, i) in vf.iter().enumerate() {
            ...
            let mut rng = rand::thread_rng();
            let q: u64 = ((self.quantity as f64 * weight[id] as f64) / (self.f_weight as f64)).ceil() as u64;
    ...
    pub fn res(&mut self) -> &Vec<GenerResult> {
        ...
        if self.i_res.len() > 0 {
            let q = (self.quantity as f64 * self.i_weight as f64 / total_weight as f64).ceil() as usize;

            let mut rng = rand::thread_rng();
            self.i_res.shuffle(&mut rng);
            for i in 0..q {
                self.res.push(GenerResult::I(self.i_res[i]));
            }
        }

        if self.f_res.len() > 0 {
            let q = (self.quantity as f64 * self.f_weight as f64 / total_weight as f64).ceil() as usize;

            let mut rng = rand::thread_rng();
            self.f_res.shuffle(&mut rng);
            for i in 0..q {
                self.res.push(GenerResult::F(self.f_res[i]));
            }
        }

        if self.s_res.len() > 0 {
            let q = (self.quantity as f64 * self.s_weight as f64 / total_weight as f64).ceil() as usize;

            let mut rng = rand::thread_rng();
            self.s_res.shuffle(&mut rng);
            for i in 0..q {
                self.res.push(GenerResult::S(self.s_res[i]));
            }
        }
